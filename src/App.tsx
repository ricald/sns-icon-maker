...
export default function App() {
  const CANVAS_SIZE = 300; // プレビュー用のキャンバスサイズ
  const OUTPUT_SIZE = 400; // 出力画像のサイズ（400x400の高品質）
  ...
  // ズームと位置の更新ロジックを共通化
  const updateScaleAndPosition = (newScale) => {
    // 最小・最大の範囲内に収める
  ...
  useEffect(() => {
    ...
    // オーバーレイを描画（画像がある場合のみ）
    if (image) {
      // 1. 丸の外側（四角いエリアの残り部分）だけを半透明の白で覆う
      ctx.fillStyle = 'rgba(255, 255, 255, 0.55)'; // やや白いオーバーレイ
      ctx.beginPath();
      // キャンバス全体の四角形を描く
      ctx.rect(0, 0, CANVAS_SIZE, CANVAS_SIZE);
      // 中央の円形を反時計回り（true）に描いてくり抜くパスを作成
      ctx.arc(CANVAS_SIZE / 2, CANVAS_SIZE / 2, CANVAS_SIZE / 2, 0, Math.PI * 2, true);
      ctx.fill();

      // 2. 円の境界線をくっきり白・グレーで描画して見やすくする
      ctx.strokeStyle = 'rgba(0, 0, 0, 0.1)'; // 外側に極薄い黒枠
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.arc(CANVAS_SIZE / 2, CANVAS_SIZE / 2, CANVAS_SIZE / 2 - 0.5, 0, Math.PI * 2);
      ctx.stroke();

      ctx.strokeStyle = 'rgba(255, 255, 255, 0.9)'; // 内側にしっかりした白枠
      ctx.lineWidth = 2;
      ctx.beginPath();
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      if (!image) return;
      e.preventDefault(); // 画面のスクロールを完全に防ぐ
      const zoomFactor = e.deltaY > 0 ? 0.95 : 1.05; // 5%ずつ拡縮
  ...
  // ズーム操作
  const handleZoomChange = (e) => updateScaleAndPosition(parseFloat(e.target.value));
  const zoomIn = () => updateScaleAndPosition(scale * 1.01);
  const zoomOut = () => updateScaleAndPosition(scale * 0.99);

  // 画像の書き出し
  const downloadImage = (isTransparent) => {
    if (!image) return;
